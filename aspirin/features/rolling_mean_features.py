...
def rolling_mean_multiseries(multiseries, lags=[6, 12, 18, 24]):
    '''
    '''
    ...
    for window in rolling_windows:
        rolled_series = multiseries[unrolled_cols].rolling(window).mean()
        rolled_series.columns = ['{}_ma_{}'.format(c, window) for c in unrolled_cols]
